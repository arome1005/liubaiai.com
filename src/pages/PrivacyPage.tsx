  ...
  return (
    <div className="page">
      ...
      <section style={{ maxWidth: 860, lineHeight: 1.8 }}>
        ...
        <p>
          「留白写作」以<strong>浏览器端</strong>为主：作品、章节正文、圣经、概要等内容默认保存在你本机浏览器的
        ...
        <p>
          在未启用云同步或未调用云端 AI 时，正文默认不经过我们的业务服务器存储。若你启用<strong>写作数据云同步</strong>，作品/章节/圣经等可能加密同步至第三方托管（如 Supabase），仍建议定期使用应用内导出备份。
          当你使用<strong>云端 AI 提供方</strong>（如 OpenAI / Claude / Gemini 等）并点击「生成」时，本次提示词（由你选择注入的内容组成）
          会通过网络发送到对应第三方服务。
        </p>
        <p>
          你可以在「设置 → AI 隐私与上传范围」中明确选择是否允许使用云端提供方、以及允许上传哪些内容（正文/圣经/摘要/摘录/RAG 检索片段等）。
          其中「RAG」包括对<strong>参考库（藏经）</strong>的命中片段，以及在侧栏开启时可选的<strong>本书圣经导出分块</strong>与<strong>本书章节正文分块</strong>（均在本地检索；仅在启用云端 AI 且你允许上传对应范围时，相关片段会随请求发送至所选模型提供方）。
