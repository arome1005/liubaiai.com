...
export function TuiyanPlanningUnifiedPanel({
  ...
  const disabledBase = !workId || useSamplePreview || planningBusyLevel !== null
  ...
  const primaryAction = (() => {
    ...
    if (planningSelectedNode.level === "outline") {
      ...
      const allDone = volumeCountForActiveOutline >= planningScale.volumeCount
      return {
        label:
          planningBusyLevel === "volume"
            ? "生成中..."
            : allDone
              ? `已完成全部 ${planningScale.volumeCount} 卷卷纲`
              : `生成第 ${nextVol} 卷卷纲`,
      ...
      {advancedOpen && (
        <div className="space-y-2 rounded-lg border border-border/30 bg-background/25 p-2.5">
          ...
          <div className="space-y-1.5">
            <p className="text-[10px] font-medium text-muted-foreground">规模设置</p>
        ...
        <div className="flex items-center justify-between text-[10px] text-muted-foreground">
          ...
          {planningActiveOutline ? (
            <span className="tabular-nums">
              {volumeCountForActiveOutline}/{planningScale.volumeCount} 卷已生成
            </span>
          ) : (
