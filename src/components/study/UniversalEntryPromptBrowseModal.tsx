...
import { listApprovedPromptTemplates } from "../../db/repo";
...
export type UniversalEntryPromptBrowseModalProps = {
  open: boolean;
...
/**
 * 万能词条类提示词浏览（与人设浏览同构，类型为 universal_entry）
 */
export function UniversalEntryPromptBrowseModal(props: UniversalEntryPromptBrowseModalProps) {
    ...
    void (async () => {
      ...
      try {
        const [m, approved] = await Promise.all([
          loadGlobalPromptTemplatesMergedByTypes(["universal_entry"], WRITER_SLOTS),
          listApprovedPromptTemplates(),
        ]);
        if (cancelled) return;
        setMerged(m);
        setFeaturedOnly(approved.filter((t) => t.type === "universal_entry" && t.status !== "rejected"));
      } finally {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader className="shrink-0 border-b border-border px-4 py-3 text-left">
          <DialogTitle className="text-base font-semibold tracking-tight">浏览万能词条类提示词</DialogTitle>
          <p className="text-xs font-normal text-muted-foreground">
            提示词库 · 类型「万能词条」、适用于写作侧栏；选一条作为本次生成的 system 指令
          </p>
        </DialogHeader>
        ...
        <div className="flex shrink-0 gap-0 border-b border-border px-2">
          {(Object.keys(TAB_LABEL) as SortTab[]).map((k) => (
            ...
            >
              {TAB_LABEL[k]}
              {tab === k && <span className="bg-primary/80 absolute bottom-0 left-3 right-3 h-0.5 rounded-full" />}
            </button>
          ))}
        </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-2 py-12 text-center text-sm text-muted-foreground">
              ...
              <p className="text-muted-foreground max-w-[240px] text-xs">
                请到「落笔 → 提示词库」新建，类型选「万能词条」，并勾选写作适用范围。
              </p>
            </div>
          ) : (
