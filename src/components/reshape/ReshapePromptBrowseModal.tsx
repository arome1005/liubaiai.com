...
export type ReshapePromptBrowseModalProps = {
  open: boolean;
...
export function ReshapePromptBrowseModal(props: ReshapePromptBrowseModalProps) {
    ...
    void (async () => {
      ...
      try {
        const [m, approved] = await Promise.all([
          loadGlobalPromptTemplatesMergedByTypes(["book_split"], WRITER_SLOTS),
          listApprovedPromptTemplates(),
        ]);
        if (cancelled) return;
        setMerged(m);
        setFeaturedOnly(approved.filter((t) => t.type === "book_split" && t.status !== "rejected"));
      } finally {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader className="shrink-0 border-b border-border px-4 py-3 text-left">
          <DialogTitle className="text-base font-semibold tracking-tight">浏览重塑类提示词</DialogTitle>
          <p className="text-xs font-normal text-muted-foreground">
            提示词库 · 类型「重塑」，可作为重塑分析的 system 指令
          </p>
        </DialogHeader>
        ...
        <div className="flex shrink-0 gap-0 border-b border-border px-2">
          {(Object.keys(TAB_LABEL) as SortTab[]).map((k) => (
            ...
            >
              {TAB_LABEL[k]}
              {tab === k ? <span className="bg-primary/80 absolute bottom-0 left-3 right-3 h-0.5 rounded-full" /> : null}
            </button>
          ))}
        </div>
