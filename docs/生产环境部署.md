...
## Supabase Auth：Site URL、Redirect URLs 与邮件

与 **`master-checklist` §B 步 7**（Redirect URLs / 邮件 / 生产环境变量）对应；**上线前**在 Supabase 控制台与本节一并核对。

1. **Dashboard** → **Authentication** → **URL Configuration**  
   - **Site URL**：填生产站点根地址（例如 `https://www.example.com`），与最终用户访问域名一致。  
   - **Redirect URLs**：加入允许的完整回调 URL。至少应包含与本应用路由一致的地址，例如 `https://你的域名/login`、`https://你的域名/reset-password` 等（以 `src/App.tsx` 路由为准）。本地开发可额外加入 `http://localhost:5173/**`（端口与 Vite 一致）。  
   若使用 Magic Link、OAuth 或邮件内重置链接，**未**列入的 URL 会导致重定向被拒。  
2. **邮件**：验证码、重置密码等依赖 **Node 后端** `POST /api/...` 可达且 **SMTP / `MAIL_*`** 已按 `backend/.env.example` 配置（见上文「后端」节）。
