...
> **冲突规则**（总体规划 §3.5.3）：用户显式 **本书锦囊 / 风格卡** 优先于泛化 **标签 profile**（标签未接入 AiPanel 前仅作规划）。
...
| B   | `user` 前段 | **上下文**：作品名、章名（受隐私开关）、文风锚点、笔感样本、故事背景、角色/关系、**本章锦囊**（目标/禁止/视角/场景）、技能预设、**术语表**（元数据许可）、关联摘录 |
| B   | `user` 中段 | 最近章概要、**全书锦囊 Markdown**（可选）、RAG 命中片段                                                      |
...
- **不上传**：章正文、全书锦囊全文、RAG（MVP 未接）；与写作侧栏 **场景不同**，勿混用 `buildWritingSidepanelMessages`。
...
- **SSE 真流式**：OpenAI 兼容路径、Ollama、**Anthropic Messages**（`stream: true`，`content_block_delta` / `text_delta`）、**Gemini**（`:streamGenerateContent?alt=sse`，按块 `geminiGenerateTextFromJson` 抽增量）。  
- **非流式**：仍可用 `generateWithProvider`（如探测、非侧栏一次性调用）。
...
| 日期         | 摘要                                             |
| ---------- | ---------------------------------------------- |
| 2026-04-07 | §3：Claude / Gemini 侧栏路径已接真流式（`providers.ts`）   |
