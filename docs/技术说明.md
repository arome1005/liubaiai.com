...
| 本书锦囊护栏（4.1～4.8）  | Dexie **schema v9**：`bibleCharacters`、`bibleWorldEntries`、`bibleForeshadowing`、`bibleTimelineEvents`、`bibleChapterTemplates`、`chapterBible`（单章约束）、`bibleGlossaryTerms`；路由 `**/work/:workId/bible`**；`exportBibleMarkdown`；整库备份 zip 含上述表；编辑器侧栏 **本章约束** + **术语命中**（字面包含） |
...
- **本书锦囊行类型**：`BibleCharacter`、`BibleWorldEntry`、`BibleForeshadow`、`BibleTimelineEvent`、`BibleChapterTemplate`、`ChapterBible`、`BibleGlossaryTerm`（见 `src/db/types.ts`）。
...
- **版本历史（摘要）**：v5 快照裁剪；v6 倒排与摘录；v7 `referenceLibrary.category`；**v8** 标签表、摘录关联创作、chunks 复合索引、摘录扩展字段；**v9** 本书锦囊护栏七表；**v10** 参考库章节检测与 `referenceChapterHeads`。
