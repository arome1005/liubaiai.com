...
- **已定案管线**：**藏经**（本地参考书，不上云存正文）→ **推演**（大纲/卷纲/细纲/导图/文策/日志，关联藏经为「提炼」非洗稿）→ **定稿** → **生辉**（按纲仿写正文，消费纲+文策+**本书锦囊**）。
...
- 长篇小说 / 同人创作：作品、卷、章、本书锦囊（设定护栏）一体化
- 顶栏 **七大功能模块 + 设置**：从「书库」到「推演纲」再到「按纲生辉」
- **数据主权**：参考库（藏经）正文 **本地**；写作与本书锦囊可选上云（Hybrid）
...
- 长线创作：**设定与伏笔** 易散 — 用 **本书锦囊** 与后续摘要/RAG 规划缓解
- AI 协作：**窗口与记忆** 有限 — 滚动摘要、本书锦囊注入、按需检索（路线图 5.x）
...
- **生辉**：**仿写** — 严格按已定稿 **纲 + 文策**，并挂载 **本书锦囊**（本作设定为准）
...
| 落笔  | `/work/:id/bible` | 本书锦囊、风格卡、设定护栏          |
| 生辉  | `/sheng-hui`      | 按纲仿写正文               |
| 藏经  | `/reference`      | 本地参考书库               |
...
- **输入**：已定稿纲、文策、**本书锦囊**、风格卡（数据已有，调用链规划中）
- **不做**：脱离纲的闲聊式长文生成（产品定调）
...
- **冲突规则**：本书锦囊/风格卡 **优先于** 泛化「标签 profile」
- 状态：**结构化本书锦囊底座已有**；模板库、词典联动等为 Backlog
...
- **Hybrid**：配置 `VITE_SUPABASE_*` 时，作品/章/本书锦囊等 **可同步云端**；**藏经仍本地**
...
- **AppShell**：顶栏七链接 + 账户 + 设置齿轮；**作品子栏**（写作/概要/本书锦囊/作品库）
