...
> **真源**：`docs/总体规划-路线图与导航整合.md`（§10～§11）、`docs/总体规划-改进版-2026-04.md`（**目标修订 + 双轨说明**）、`docs/UI-v0-对齐任务清单.md`（**各功能区功能 vs 向 v0 皮 进度**）、`docs/bug-backlog.md`（**规划级缺陷 / 分期**）、`docs/路线图.md`、`design/implementation-steps.md`、`**docs/留白写作-深度改进建议书v1.md`（§F 深度改进对标，与 §B 并行）**。**  
...
- ~~第 4 组（4.1～4.8）圣经护栏（人物/世界观/伏笔/时间线/章模板等）与导出~~
...
- ~~写作侧栏：`buildWritingSidepanelMessages` + `buildWritingSidepanelInjectBlocks`（作品/章、风格卡、本章圣经、变量、摘录、邻章概要、全书圣经、RAG、正文/概要/选区、任务句；云端隐私门控与截断与预览一致）~~
- ~~作品 **留白标签 → tagProfileText** 接入数据层并注入（`Work.tags` + `work-tags.ts` → `AiPanel` / `assemble-context`）~~
- 邻章摘要 / 圣经 **勾选子集** 等更细粒度装配（可选增强）
...
- ~~写作页 AiPanel：可折叠「本次使用材料（简版）」列表（作品/章、模型、风格卡、本章圣经、全书圣经、摘录、概要、RAG、云端隐私摘要、粗估 token；标签行占位「暂无」）~~
...
- 每章或每 N 章**自动**触发、后台队列（可选，待产品与成本策略）
...
- ~~**设计真源**：`docs/rag-index-design.md`（参考库已落地；圣经/正文/向量为规划增量）~~

### 步 24 — RAG 检索 + 注入

- ~~**参考库 MVP**：侧栏 top-k、hybrid/strict、`ragHits` → `assemble-context` 注入~~
- ~~圣经 / 章节正文多源检索与合并注入（**运行时** 分块 + 混合打分，见 `docs/rag-index-design.md` §2、`src/util/work-rag-runtime.ts`；持久倒排索引可选二期）~~
...
- ~~**规则 MVP（先规则后 AI）**：`/logic` 选作品与章节，**扫描本章正文** vs **本章圣经禁写**、**全书风格卡禁用套话**、**术语表 category=dead**、**人物禁忌**；实现 `src/util/bible-consistency-scan.ts`（`runBibleConsistencyScan` / `splitConstraintPhrases`）+ `LogicPage` + `index.css` `.logic-*` 样式~~
- 基于 **LLM** 的语义矛盾、**全书/多章**批量扫描、命中 **忽略/白名单**（可选，与步 33+ 交织）
...
- 一键写入写作侧栏草稿、与装配器/文策日志联动、多轮改纲（后续）

### 步 34 — 推演：时间轴/地图极简 MVP

- ~~**MVP**：`/logic` 圣经时间线列表 + 世界观条目速览（只读）+ 时间线快速追加（标签/备注/可选关联章），链到 `/work/:id/bible`；`LogicPage` + `index.css` `.logic-timeline-*` / `.logic-world-*` 样式~~
- 独立「地点-事件」表、地图视图、与三分支/扫描的深联动（后续）
...
- ~~**MVP**：每作品 `writingPromptTemplates`（IndexedDB v15 + Supabase `writing_prompt_template`）；圣经页「提示词」Tab CRUD/排序；「去写作装配」→ 写作页打开 AI 侧栏并覆盖「额外要求」~~

### 步 43 — 落笔：笔感样本

- ~~**MVP**：每作品 `writingStyleSamples`（IndexedDB v16 + Supabase `writing_style_sample`）；圣经「笔感」Tab；正文注入 `buildWritingSidepanelCtxParts`（user 上下文，紧接文风锚点）~~

### 步 44 — 落笔：全局词典 → 生成

- ~~**MVP**：圣经术语表经 `buildWritingSidepanelCtxParts` 注入 user 上下文（本章约束与关联摘录之间）；词条按字数降序；云端与 `allowMetadata` 同档~~
...
- 会话跨设备同步、与推演文策日志联动、专用成本确认（可选，后续）
...
- 整卷仿写 / 多章批量等独立「始终确认」清单（可与 `implementation-steps` C.6.3 合并，后续）
...
| ~~**UI-8**~~  | ~~圣经 / 落笔页~~                                              | ~~`BiblePage.tsx`~~                                         | ~~`luobi-module.tsx`~~                                      | ~~与 **步 42～44** 数据功能可后续加~~ **✓ 已完成**：顶栏书名与统计、快捷链与导出、分区 tab（`toolbar-seg` 风格 + 数量角标）、`bible-section-panel` 与卡片 `focus-within`；数据与导出逻辑未改                                                                                              |
...
- **步 14 相关（小步）**：**圣经 / 概要** 子路由隐藏 `**app-topbar`**（`isWorkBibleOrSummaryPath`）。  
...
- **E.2.7** `LogicPage`、`InspirationPage`、`ChatPage`  
- **E.2.8** `ShengHuiPage`（**生辉** 功能 MVP 已具；**整页** v0 级同源渲染仍待本项勾选）
...
- ~~**Layer 1 静态圣经**~~：装配器已注入全书/本章圣经等（步 9/10）  
...
- ~~**Context 体积与可预期性**~~：`approx-tokens`、材料简版（步 15/16）、超限确认、步 27「精简并重试」；**后续**：侧栏 **Token 预算可视化（色块）**、**显式优先级截断链**（与 `docs/ai-context-merge-order.md` 合并落地）、圣经注入前 **压缩版** 与「全量仅按需」（报告 §1.2）  
- **生成与人类改写融合**（报告 §1.3）：侧栏草稿与正文分离 + diff（步 11/12）已降低覆盖风险；**后续**：不可见版本标记、续写前检测「用户已改写」、可选 **风格锚定段**  
- ~~**一致性守门**~~：步 32 **规则**扫描 MVP；**后续**：生成后 **LLM 扫描**、存疑标记、与圣经 **一键对照**（报告 §1.4）  
- ~~**推演三分支**~~：步 33；**后续**：**推演漏斗**（倾向 / 采纳正稿 / 归档流光）、推演 **历史时间轴**（报告 §1.5）

### F.2 现有功能深化（报告 §二）

- ~~**圣经**~~：全链路 + 提示词/笔感/术语（步 41～44）；**决策 #4**：健康度等指标 **一期不展示**；**后续**：若产品重开—**健康度评分**、**按类型推荐字段**、**圣经字段版本历史**、笔感 **量化对比**（报告 §2.1）  
- ~~**藏经 RAG**~~：索引、阅读器、混合检索、本地安全锁（步 23/24/39～41）；**后续**：召回 **置信度/可解释**、写作中 **相关条浮现**、阅读器 **分层标注**、跨作品共享（报告 §2.2）  
- ~~**问策**~~：步 46 MVP；**后续**：会话 **标签**、结论 **转圣经/流光**、与推演 **一键联动**（报告 §2.3）

### F.3 留白页（报告 §三）

- ~~**首页 / 作品库**~~：Hub、继续创作、统计与 §E.2.2 皮；**后续**：**今日写作快速入口**（上次摘要一眼、今日字数目标）、卡片 **信息密度**（相对时间、章进度、圣经简况）、**创作数据面板**（ streak、各书占比等，本地即可）、全站 **灵感快录入口**、长期未更 **温和提醒**（报告 §3）
...
| 2026-04-02 | **UI-8**：`BiblePage` 顶栏与分区 tab（`bible-tab-seg` / `.is-on`）、`tabCounts` 角标、`bible-section-panel`；`index.css` 圣经区块与 `.bible-card:focus-within`                                                                     |
...
| 2026-04-02 | **UI-1 补充**：藏经入 `hubPaths`；顶栏 **Ctrl/⌘+K** 打开作品库 + 平台化快捷键文案；**§D.4** 增录 **圣经/概要子页多头顶栏** 待步 14/改版收敛                                                                                                               |
| 2026-04-02 | **UI-0 首轮 token** + **圣经/概要** 去掉薄 `app-topbar`（`workPath.isWorkBibleOrSummaryPath`）；**§D.4** 对应行改为已减一层                                                                                                           |
...
| 2026-04-02 | **步 34（时间轴极简）**：推演页时间线/世界观面板 + 样式；圣经同源编辑出口                                                                                                                                                                       |
...
| 2026-04-02 | **步 24**：本书圣经 / 正文 **运行时** RAG（`work-rag-runtime.ts`）+ 侧栏多源勾选；`docs/rag-index-design.md` §2～3                                                                                                                    |
